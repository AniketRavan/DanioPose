...
from programsForDrawingImage import *
    ...
    def draw(self):
        ...
        if self.annotationsType == AnnotationsType.segmentation:
            for viewIdx in range(3):
                ...
                self.contourContainer[viewIdx] = np.squeeze(contours[0])
...
class Aquarium:
    ...
    def __init__(self):
        ...
        # Check if the lookup tables are actually used
        ...
        for viewIdx in range(1,3):
            ...
            for fishIdx, fish in enumerate(self.fishList):
                ...
                if topPoint <= self.waterLevel and not(topPoint <= 0):
                    ...
                    reflectedFish.contourContainer[viewIdx] = np.squeeze(contour[0])
                    allG[fishIdx,...] = gray
...
if __name__ == "__main__":
    fish = Fish(4, np.random.random((22,1)))
    print(fish)
